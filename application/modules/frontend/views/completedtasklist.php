...
<div id="page-wrapper">
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">Completed Task List </h1>
        </div>
        <!-- /.col-lg-12 -->
    </div>
    <!-- row -->
                        ...
                        <table class="table table-bordered display nowrap" cellspacing="0" width="100%" id="appointment">
                            ...
                            <tbody id="table_body">
                                ...
                                <tr class="odd gradeX" id="tr_<?php echo $count;?>" style="background: lightgreen">
                                    ...
                                    <td>
                                        <?php if(!empty($value->screenshot_image)){ ?>
                                        <a download="<?php echo $value->screenshot_image ?>" href="<?php echo base_url()."asset/completed_task/".$value->screenshot_image; ?>" title="ImageName">
                                       
                                        <img src="<?php echo base_url()."asset/completed_task/".$value->screenshot_image; ?>" height="50px" width="50px;">
                                        </a> 
                                        <?php }else{
...
function delete_task(id,tr_id) {
    ...
    function get_data(){
        ...
        $.ajax({
                url: "<?php echo base_url('superadmin/get_completed_task')?>",
                method: "POST",
                dataType: "json",
                data: {
                     ...
                     if (response.length > 0) {
                         ...
                         for (var i = 0; i < response.length; i++) {
                           ...
                           // var del = "delete_leads("+response[i].id+","+i+")";
                            $('#table_body').append('<tr class="odd gradeX" id="tr_'+i+'"><td>'+j+'</td><td>'+assign+'</td><td>'+response[i].username+'</td><td>'+response[i].name+'</td><td>'+response[i].time+'Days</td><td>'+response[i].url+'</td><td>'+response[i].description+'</td><td>'+response[i].created_at+'</td><td>'+response[i].completed_date+'</td></tr>');
